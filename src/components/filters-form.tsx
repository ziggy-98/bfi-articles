...
const FiltersForm = ({ updateFilters }: Props) => {
  ...
  useEffect(() => {
    if (authors.length === 0) {
      fetch("https://content-store.explore.bfi.digital/api/authors")
        .then((res) => res.json())
        .then((res) => {
          setAuthors(res.data);
        });
    ...
    if (types.length === 0) {
      fetch("https://content-store.explore.bfi.digital/api/types")
        .then((res) => res.json())
        .then((res) => {
          setTypes(res.data);
        });
  ...
  return (
    <form className="filter-form" onSubmit={handleSubmit}>
      <Grid item xs={12} sm={6} md={4} className="author-filter">
        <FormControl fullWidth>
          <Autocomplete
            ...
            options={authors}
            renderInput={(params) => (
              <TextField {...params} label="Filter by author" />
            )}
            onChange={(e, value) => setAuthor(value ? value.id : "")}
          />
        </FormControl>
      </Grid>
      ...
      <Grid item xs={12} sm={12} md={4} className="filter-submit">
        <Button
          sx={{ fontWeight: 600, padding: "6px 30px", fontSize: "1rem" }}
